...
from fixrsearch.search_script.utils import extract_apk
...
def main():
  ...
  if use_groum_file:
    ...
    graphs_zip_name = extract_apk(input_file, graph_extractor_jar,
                                  graphs_zip_file.name,
                                  commit_ref.repo_ref.user_name,
                                  commit_ref.repo_ref.repo_name,
                                  commit_ref.commit_hash)
    if graphs_zip is None:
      logging.debug("Error in the extraction of the grapsh from the APKs")
    else:
      assert graphs_zip_name == graphs_zip_file.name
      try:
        try:
          # Call the search
          wp.decompress(graphs_zip, graph_path)
          index = GroumIndexBase(graph_path)
          index.build_index()
          pr_processor = PrProcessor(index, search, src_client)
          anomalies = pr_processor.process_graphs_from_commit(commit_ref, None,
                                                              None)
        finally:
